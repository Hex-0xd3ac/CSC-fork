...
export async function reloadModule(hazel, core, hold, socket, line) {
    ...
    // 发送重载完成消息
    core.replyInfo("ROOT", `模块 ${moduleName} 重载完成，当前版本为 ${hazel.loadHistory.get(modulePath)}。`, socket);
}
export async function reloadModuleByID(hazel, core, hold, socket, line) {
    ...
    if (versionID > hazel.loadIDMax.get(modulePath)) {
        core.replyInfo("ROOT", "模块 " + moduleName + " 的版本 " + versionID + " 不存在。", socket);
        return;
    }
    ...
    // 重载指定模块到指定版本
    core.replyInfo("ROOT", `模块 ${moduleName} 重载请求已接收，目标版本为 ${versionID}。`, socket);
    await hazel.reloadModuleByID(modulePath, versionID);
    // 发送重载完成消息
    core.replyInfo("ROOT", `模块 ${moduleName} 重载完成，已恢复至版本 ${versionID}。`, socket);
}
