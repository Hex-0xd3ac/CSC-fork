...
export default class Hazel extends EventEmitter2 {
    ...
    async reloadModule(modulePath) {
        let loadID = this.loadHistory.get(modulePath) !== undefined
        ...
        this.loadIDMax.set(modulePath, this.loadIDMax.get(modulePath) === undefined
            ? this.functionLoadID
            : Math.max(loadID, this.loadIDMax.get(modulePath)));
    }
    async reloadModuleByID(modulePath, loadID) {
        let module = await importModule(modulePath, loadID);
        this.loadHistory.set(modulePath, loadID);
        this.loadedFunctions.set(module.name, module);
        this.loadIDMax.set(modulePath, this.loadIDMax.get(modulePath) === undefined
    ...
    async loadModules(forceLoad) {
        let result = (await loadModule(this, this.mainConfig.baseDir + this.mainConfig.hazel.moduleDirs.initsDir, "init", ++this.initLoadID));
        ...
        let { moduleList: loadedFunctions, existError: functionExistError } = (await loadModule(this, this.mainConfig.baseDir + this.mainConfig.hazel.moduleDirs.functionsDir, "function", ++this.functionLoadID));
